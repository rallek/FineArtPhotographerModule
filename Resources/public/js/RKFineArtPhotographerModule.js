...
function rKFineArtPhotographerCapitaliseFirstLetter(string) {
 ...
 */
function rKFineArtPhotographerInitQuickNavigation() {
    var quickNavForm;
    var objectType;

    if (jQuery('.rkfineartphotographermodule-quicknav').length < 1) {
        return;
    }

    quickNavForm = jQuery('.rkfineartphotographermodule-quicknav').first();
    objectType = quickNavForm.attr('id').replace('rKFineArtPhotographerModule', '').replace('QuickNavForm', '');

    quickNavForm.find('select').change(function (event) {
        quickNavForm.submit();
    });

    var fieldPrefix = 'rkfineartphotographermodule_' + objectType.toLowerCase() + 'quicknav_';
 ...
 */
function rKFineArtPhotographerSimpleAlert(anchorElement, title, content, alertId, cssClass) {
    var alertBox;
 ...
 */
function rKFineArtPhotographerInitMassToggle() {
    if (jQuery('.rkfineartphotographer-mass-toggle').length > 0) {
        jQuery('.rkfineartphotographer-mass-toggle').unbind('click').click(function (event) {
            jQuery('.rkfineartphotographer-toggle-checkbox').prop('checked', jQuery(this).prop('checked'));
        });
    }
}
 ...
 */
function rKFineArtPhotographerInitInlineWindow(containerElem) {
    var newWindowId;
    var modalTitle;
 ...
 */
function rKFineArtPhotographerInitQuickViewModals() {
    jQuery('.rkfineartphotographer-inline-window').each(function (index) {
        rKFineArtPhotographerInitInlineWindow(jQuery(this));
    });
}
...
jQuery(document).ready(function () {
    ...
    isViewPage = jQuery('.rkfineartphotographermodule-view').length > 0;
    isDisplayPage = jQuery('.rkfineartphotographermodule-display').length > 0;

    rKFineArtPhotographerInitImageViewer();

    if (isViewPage) {
        rKFineArtPhotographerInitQuickNavigation();
        rKFineArtPhotographerInitMassToggle();
        rKFineArtPhotographerInitItemActions('view');
    } else if (isDisplayPage) {
        rKFineArtPhotographerInitItemActions('display');
    }

    rKFineArtPhotographerInitQuickViewModals();
});
