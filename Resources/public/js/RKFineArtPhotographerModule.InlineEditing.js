 ...
 */
function rKFineArtPhotographerCreateInlineEditingWindowInstance(containerElem) {
    var newWindowId;
 ...
 */
function rKFineArtPhotographerInitInlineEditingWindow(objectType, idPrefix, containerId, inputType) {
    ...
    jQuery.each(rKFineArtPhotographerInlineEditHandlers, function (key, editHandler) {
        ...
        editHandler.windowInstanceId = rKFineArtPhotographerCreateInlineEditingWindowInstance(jQuery('#' + containerId));
    });
    ...
    newEditHandler = {
        ...
        inputType: inputType,
        windowInstanceId: rKFineArtPhotographerCreateInlineEditingWindowInstance(jQuery('#' + containerId))
    };

    // add it to the list of edit handlers
    rKFineArtPhotographerInlineEditHandlers.push(newEditHandler);
}

/**
 * Creates a link for editing an existing item using inline editing.
 */
function rKFineArtPhotographerCreateInlineEditLink(objectType, idPrefix, elemPrefix, itemId) {
 ...
 */
function rKFineArtPhotographerInitInlineEditLink(objectType, idPrefix, elemPrefix, itemId, inputType) {
    jQuery('#' + elemPrefix + 'Edit').click(function (event) {
        event.preventDefault();
        rKFineArtPhotographerInitInlineEditingWindow(objectType, idPrefix, idPrefix + 'Reference_' + itemId + 'Edit');
    });
}
 ...
 */
function rKFineArtPhotographerDetermineInputReference(objectType, alias, idPrefix, inputType, targetWindow) {
    var inputPrefix, inputIdentifier, inputField;

    // determine reference to input element
    inputPrefix = targetWindow.jQuery('.rkfineartphotographer-edit-form').first().attr('name');
    inputField = null;
 ...
 */
function rKFineArtPhotographerInitInlineEditingButtons(objectType, alias, idPrefix, inputType, createUrl) {
    var inputReference, createButtonId, createButton, itemIds, itemIdsArr;

    inputReference = rKFineArtPhotographerDetermineInputReference(objectType, alias, idPrefix, inputType, window);
    ...
    if (jQuery('#' + createButtonId).length < 1) {
        ...
        createButton = jQuery('<a />', {
            ...
            class: 'btn btn-default rkfineartphotographermodule-inline-button'
        }).append(
    ...
    createButton.click(function (event) {
        event.preventDefault();
        rKFineArtPhotographerInitInlineEditingWindow(objectType, idPrefix, createButtonId, inputType);
    });
    ...
    jQuery.each(itemIdsArr, function (key, existingId) {
        ...
        if (existingId) {
            ...
            jQuery('#' + elemPrefix).click(function (event) {
                event.preventDefault();
                rKFineArtPhotographerInitInlineEditingWindow(objectType, idPrefix, elemPrefix, inputType);
            });
        }
    });
}
 ...
 */
function rKFineArtPhotographerCloseWindowFromInside(idPrefix, itemId, formattedTitle, searchTerm) {
    ...
    jQuery.each(window.parent.rKFineArtPhotographerInlineEditHandlers, function (key, editHandler) {
        ...
        inputReference = rKFineArtPhotographerDetermineInputReference(editHandler.objectType, editHandler.alias, idPrefix, inputType, window.parent);
        if (null === inputReference || null === inputReference.field) {
            return;
        }
        ...
        window.parent.rKFineArtPhotographerSimpleAlert(anchorElement, window.parent.Translator.__('Information'), window.parent.Translator.__('Action has been completed.'), 'actionDoneAlert', 'success');
