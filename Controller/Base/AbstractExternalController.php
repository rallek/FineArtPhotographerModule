...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction(Request $request, $objectType, $id, $source, $displayMode)
    {
        ...
        $contextArgs = ['controller' => 'external', 'action' => 'display'];
        $templateParameters = $this->get('rk_fineartphotographer_module.controller_helper')->addTemplateParameters($objectType, $templateParameters, 'controllerAction', $contextArgs);
        
        return $this->render('@RKFineArtPhotographerModule/External/' . ucfirst($objectType) . '/' . $template, $templateParameters);
    }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        $cssAssetBag->add($assetHelper->resolve('@RKFineArtPhotographerModule:css/style.css'));
        ...
        if (!in_array($objectType, $activatedObjectTypes)) {
            ...
            $redirectUrl = $this->get('router')->generate('rkfineartphotographermodule_external_finder', ['objectType' => array_shift($activatedObjectTypes), 'editor' => $editor]);
        
            return new RedirectResponse($redirectUrl);
        }
        
        if (!$this->hasPermission('RKFineArtPhotographerModule:' . ucfirst($objectType) . ':', '::', ACCESS_COMMENT)) {
            throw new AccessDeniedException();
        }
        ...
        $form = $this->createForm('RK\FineArtPhotographerModule\Form\Type\Finder\\' . ucfirst($objectType) . 'FinderType', $templateParameters, $formOptions);
        ...
        if ($searchTerm != '') {
            $qb = $this->get('rk_fineartphotographer_module.collection_filter_helper')->addSearchFilter($objectType, $qb, $searchTerm);
        }
        ...
        $output = $this->renderView('@RKFineArtPhotographerModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);
