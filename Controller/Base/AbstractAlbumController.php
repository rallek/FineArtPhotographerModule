...
abstract class AbstractAlbumController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('rkfineartphotographermodule_album_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'rkfineartphotographermodule_album_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
        ...
        foreach ($templateParameters['items'] as $album) {
            if (!$this->hasPermission('RKFineArtPhotographerModule:' . ucfirst($objectType) . ':', $album->getKey() . '::', $permLevel)) {
                continue;
            }
    ...
    protected function displayInternal(Request $request, AlbumEntity $album, $isAdmin = false)
    {
        ...
        $featureActivationHelper = $this->get('rk_fineartphotographer_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
            if (!$this->get('rk_fineartphotographer_module.category_helper')->hasPermission($album)) {
                throw new AccessDeniedException();
            }
        }
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('rk_fineartphotographer_module.form.handler.album');
    ...
    protected function deleteInternal(Request $request, AlbumEntity $album, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'RKFineArtPhotographerModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'album', 'id' => $album->getKey()];
        ...
        $redirectRoute = 'rkfineartphotographermodule_album_' . ($isAdmin ? 'admin' : '') . 'view';
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'RKFineArtPhotographerModule', 'user' => $userName, 'action' => $action, 'entity' => 'album', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'RKFineArtPhotographerModule', 'user' => $userName, 'entity' => 'album', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'RKFineArtPhotographerModule', 'user' => $userName, 'action' => $action, 'entity' => 'album', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('rkfineartphotographermodule_album_display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('rkfineartphotographermodule_album_' . ($isAdmin ? 'admin' : '') . 'index');
    }
    ...
    public function handleInlineRedirectAction($idPrefix, $commandName, $id = 0)
    {
        ...
        $searchTerm = '';
        if (!empty($id)) {
            $repository = $this->get('rk_fineartphotographer_module.entity_factory')->getRepository('album');
            $album = $repository->selectById($id);
            if (null !== $album) {
                $formattedTitle = $this->get('rk_fineartphotographer_module.entity_display_helper')->getFormattedTitle($album);
                $searchTerm = $album->getAlbumTitle();
            }
        }
        ...
        return new PlainResponse($this->get('twig')->render('@RKFineArtPhotographerModule/Album/inlineRedirectHandler.html.twig', $templateParameters));
    }
}
