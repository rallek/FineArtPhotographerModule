...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
    ...
    public function contentGetTypes(\Zikula_Event $event)
    {
        ...
        $types->add('RKFineArtPhotographerModule_ContentType_Item');
        
        // plugin for showing a list of multiple items
        $types->add('RKFineArtPhotographerModule_ContentType_ItemList');
    }
     ...
     * RKFineArtPhotographerModule will use this to add a javascript helper to add custom items.
     *
     * You can access general data available in the event.
     *
     * The event name:
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $targetDir = 'web/modules/rkfineartphotographer';
        $finder = new Finder();
        if (!$this->filesystem->exists($targetDir)) {
            $this->filesystem->mkdir($targetDir, 0777);
            if (is_dir($originDir = 'modules/RK/FineArtPhotographerModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/RK/FineArtPhotographerModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                'module' => 'RKFineArtPhotographerModule',
                'type' => 'javascript',
                'path' => $this->request->getBasePath() . '/web/modules/rkfineartphotographer/js/RKFineArtPhotographerModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkfineartphotographermodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkfineartphotographer/scribite/CKEditor/rkfineartphotographermodule/',
            'file' => 'plugin.js',
            'img' => 'ed_rkfineartphotographermodule.gif'
        ]);
    }
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkfineartphotographermodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkfineartphotographer/scribite/Quill/rkfineartphotographermodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkfineartphotographermodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkfineartphotographer/scribite/Summernote/rkfineartphotographermodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkfineartphotographermodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkfineartphotographer/scribite/TinyMce/rkfineartphotographermodule/plugin.js'
        ]);
    }
}
