...
namespace RK\FineArtPhotographerModule\Form\Handler\AlbumItem\Base;

use RK\FineArtPhotographerModule\Form\Handler\Common\EditHandler;
use RK\FineArtPhotographerModule\Form\Type\AlbumItemType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                ...
                $logArgs = ['app' => 'RKFineArtPhotographerModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(AlbumItemType::class, $this->entityRef, $options);
    }
    ...
    protected function getDefaultReturnUrl(array $args = [])
    {
        ...
        $routePrefix = 'rkfineartphotographermodule_' . $this->objectTypeLower . '_' . $routeArea;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'RKFineArtPhotographerModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'album item', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl(array $args = [])
    {
        if (true === $this->templateParameters['inlineUsage']) {
            ...
            return $this->router->generate('rkfineartphotographermodule_' . $this->objectTypeLower . '_handleinlineredirect', $urlArgs);
        }
        ...
        if ($this->request->getSession()->has('rkfineartphotographermodule' . $this->objectTypeCapital . 'Referer')) {
            $this->request->getSession()->remove('rkfineartphotographermodule' . $this->objectTypeCapital . 'Referer');
        }
        ...
        switch ($this->returnTo) {
            ...
            case 'adminViewAlbums':
                return $this->router->generate('rkfineartphotographermodule_album_' . $routeArea . 'view');
            case 'userOwnViewAlbums':
            case 'adminOwnViewAlbums':
                return $this->router->generate('rkfineartphotographermodule_album_' . $routeArea . 'view', ['own' => 1]);
            case 'userDisplayAlbum':
            case 'adminDisplayAlbum':
                if (!empty($this->relationPresets['album'])) {
                    return $this->router->generate('rkfineartphotographermodule_album_' . $routeArea . 'display',  ['id' => $this->relationPresets['album']]);
                }
